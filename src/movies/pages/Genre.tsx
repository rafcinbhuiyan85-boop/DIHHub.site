...
export function MoviesGenre({ genreId, onNavigate, onBack }: { genreId: string; onNavigate: (path: string) => void; onBack: () => void }) {
  ...
  const { data: movies, isLoading } = useDiscoverByGenre(genreId);
  const genreName = genreData?.genres.find(g => g.id.toString() === genreId)?.name || 'Genre';
  return (
    <div style={{ minHeight:'100vh', background:'#07090f', color:'#fff', paddingTop:60, paddingBottom:60 }}>
      <div style={{ maxWidth:1200, margin:'0 auto', padding:'0 24px' }}>
        ...
        <h1 style={{ fontSize:'clamp(30px,6vw,68px)', fontWeight:900, letterSpacing:'-2px', color:'#fff', marginBottom:32 }}>{genreName} <span style={{ color:'#F59E0B' }}>Movies</span></h1>
