...
export function MoviesSearch({ onNavigate, initialParams }: { onNavigate: (path: string) => void; initialParams?: Record<string, string> }) {
  ...
  const { data: searchResults, isLoading: searching } = useSearchMovies(debouncedQuery);
    ...
    <div style={{ minHeight:'100vh', background:'#07090f', color:'#fff', paddingTop:80, paddingBottom:60 }}>
      <div style={{ maxWidth:1200, margin:'0 auto', padding:'0 24px' }}>
        <div style={{ textAlign:'center', marginBottom:36 }}>
          <h1 style={{ fontSize:'clamp(28px,5vw,48px)', fontWeight:900, letterSpacing:'-1px', color:'#fff', marginBottom:8 }}>Find Your Next <span style={{ color:'#F59E0B' }}>Movie</span></h1>
          <p style={{ color:'rgba(255,255,255,0.4)', fontSize:14 }}>Search from thousands of movies</p>
        </div>
        <div style={{ position:'relative', maxWidth:680, margin:'0 auto 40px' }}>
          ...
          <input type="text" placeholder="Search movies, actors…" value={query} onChange={e => setQuery(e.target.value)} autoFocus style={{ width:'100%', padding:'18px 48px 18px 56px', borderRadius:16, border:'1px solid rgba(255,255,255,0.12)', background:'rgba(255,255,255,0.06)', color:'#fff', fontSize:17, outline:'none', boxSizing:'border-box', backdropFilter:'blur(20px)', transition:'all 0.3s ease', boxShadow:'0 8px 32px rgba(0,0,0,0.3)' }}/>
